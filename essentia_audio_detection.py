# Imports
from essentia.standard import FrameGenerator, DiscontinuityDetector, GapsDetector, ClickDetector
import numpy as np
import datetime


def audio_detection(audio: np.ndarray, start_time=0):
    # Normalise audio to the range [-1, 1]
    normalised_audio = audio / np.max(np.abs(audio))

    gaps = audio_gap_detection(normalised_audio, start_time)
    clicks = audio_click_detection(normalised_audio, start_time)
