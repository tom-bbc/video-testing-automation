...
from DOVER.dover.datasets import UnifiedFrameSampler, spatial_temporal_view_decomposition
...
def encode_text_prompts(prompts, device):
...
if __name__ == '__main__':
    ...
    def format_maxvqa(output):
        ...
        for i, (name, pos, neg, score) in enumerate(zip(dimension_names, positive_descs, negative_descs, output)):
            output_ += f"Axis {pos}<-->{neg} Score: {rescale(score, i)*100:.1f}\n"
            print(f"    * Metric #{i}:")
            print(f"        * Metric name:  {name}")
            print(f"        * Score range:  ({neg}) => ({pos})")
            print(f"        * Prediction:   {rescale(score, i) * 100:.2f}% ({score:.4f})")
